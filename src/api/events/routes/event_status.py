...
from db.crud.events_crud import start_event, end_event, publish_event, auto_terminate
...
@event_status_bp.patch("/auto-terminate/<int:event_id>")
def route_auto_terminate(event_id):
    try:
        event_id = int(event_id)
    except:
        return jsonify({"error": "event_id must be an integer"}), 400
    ...
    try:
        result = auto_terminate(event_id)
    except Exception as e:
        logger.error(f"Error auto terminating event: {e}")
        return jsonify(generic_error_response), 500
